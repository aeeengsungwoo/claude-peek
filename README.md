...
> **Windows only.** The popup is drawn with PowerShell + WPF. The CLI installs/uninstalls anywhere, but the mascot only appears on Windows.
...
Open `~/.claude/claude-peek/peek.ps1` and edit the values at the top:
...
- `npx claude-peek install` writes a `Stop` hook like:
  ...
  ```
- When Claude Code stops, it runs `peek.ps1`, which shows a borderless, top-most, transparent, click-through-friendly window at the top-center of the primary screen and animates the character down and back up.

## Requirements

- Windows 10/11 (PowerShell 5.1+, WPF — both ship with Windows)
